...
export default function WhyUs() {
  const { data: content } = useQuery({ queryKey: ['site-content'], queryFn: () => contentApi.get().then(r => r.data), staleTime: 0 });
    ...
    <section className="py-16 bg-white dark:bg-gray-950">
      <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
        <div className="text-center mb-12">
          <h2 className="section-title">{content?.why_us_title || 'Why Choose Food Palace?'}</h2>
          <p className="text-gray-500 dark:text-gray-400 mt-2 max-w-xl mx-auto">{content?.why_us_subtitle || 'Committed to the best Nigerian food experience'}</p>
        </div>
